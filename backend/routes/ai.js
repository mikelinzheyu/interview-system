...
router.get('/chat/stream', auth, rateLimit(30, 60), (req, res) => {
  const { message, articleContent, conversationId, postId, userId: userIdFromClient } = req.query;
  const userId = userIdFromClient || req.user?.id || 'anonymous';
  ...
  (async () => {
    try {
      ...
      try {
        ...
        // ✅ 新增：Dify API 失败，自动回退到 Mock 数据
        logger.warn('[AI/Chat] Dify API 连接失败，自动回退到 Mock 数据:', {
          error: difyError.message,
          statusCode: difyError.statusCode,
          user: userId
        });
        ...
        // ✅ 修复：保持原始的 conversationId，实现正确的多轮对话
        // 如果已有 conversationId（多轮对话），继续使用它
        // 如果没有（首次对话），生成新的临时 ID
        if (!finalConversationId) {
          finalConversationId = `conv-${postId}-${userId}-${Date.now()}`;
        }
...
router.post('/chat/stream', auth, rateLimit(30, 60), (req, res) => {
  const { message, articleContent, conversationId, postId, userId: userIdFromClient } = req.body;
  const userId = userIdFromClient || req.user?.id || 'anonymous';
  ...
  (async () => {
    try {
      ...
      try {
        ...
        // ✅ 新增：Dify API 失败，自动回退到 Mock 数据
        logger.warn('[AI/Chat] Dify API 连接失败，自动回退到 Mock 数据 (POST):', {
          error: difyError.message,
          statusCode: difyError.statusCode,
          user: userId
        });
